...
import me.khruslan.tierlistmaker.data.models.scroll.AutoScrollDirection
import me.khruslan.tierlistmaker.data.models.scroll.AutoScrollState
...
class AutoScrollManager(private val recyclerView: RecyclerView) {
     ...
     */
    private val scrollChecker = AutoScrollChecker(
    ...
    private fun computeScrollState(dragLocation: DragLocation?): AutoScrollState {
        val targetPosition = when (val target = dragLocation?.target) {
            is TierDragData -> target.tierPosition
            is ImageDragData -> target.tierPosition
            else -> return AutoScrollState.Idle
        }
        val targetOffset = dragLocation.positionInTarget.y.toInt()
        val targetLocation = TargetLocation(targetPosition, targetOffset)

        return when {
            targetPosition == BACKLOG_TIER_POSITION -> AutoScrollState.Idle
            scrollChecker.shouldScrollUp(targetLocation) -> AutoScrollState.scrollingUp()
            scrollChecker.shouldScrollDown(targetLocation) -> AutoScrollState.scrollingDown()
